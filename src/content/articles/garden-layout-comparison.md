...
**Row gardening for single-crop volume:** If you're growing 50 tomato plants, 200 feet of beans, or any crop that you'll process and preserve in bulk, row gardening is the practical format. You can use a wheel hoe for cultivation, run soaker hose down the rows cheaply, and harvest efficiently with less labor per pound than a multi-bed system. The yield-per-sq-ft disadvantage matters less when the total footprint is large and the path investment is manageable. For succession planting guidance across a row layout, see the [succession planting calendar](/guides/succession-planting-calendar/).
